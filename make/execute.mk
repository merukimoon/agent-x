...
orchestrator-validate:
	@set -eu; \
	if [ -z "${GOAL}" ]; then echo "GOAL is required. Usage: make orchestrator-validate GOAL=\"...\" [CONTEXT=\"...\"]"; exit 1; fi; \
	RUN_ID="orch-val-$$(date +%Y%m%d-%H%M%S)"; \
	echo "Starting Orchestrator Validation Run: $$RUN_ID"; \
	echo "Goal: ${GOAL}"; \
	CONTEXT_ARG=""; \
	if [ -n "${CONTEXT}" ]; then CONTEXT_ARG="--context \"${CONTEXT}\""; else \
		echo "Policy context for validation" > /tmp/context-$$RUN_ID.md; \
		CONTEXT_ARG="--context /tmp/context-$$RUN_ID.md"; \
	fi; \
	echo "1. Creating Run..."; \
	$(PNPM) run dev run new --goal "${GOAL}" $$CONTEXT_ARG --run "$$RUN_ID" --quiet; \
	echo "2. Running Planner..."; \
	$(PNPM) run dev plan --run "$$RUN_ID"; \
	echo "3. Executing Plan..."; \
	$(PNPM) run dev run --run "$$RUN_ID"; \
	echo "4. Validating Run $$RUN_ID..."; \
