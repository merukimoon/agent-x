...
function main() {
    ...
    if (!name) {
        fail('NAME is required. Example: make run-new NAME="docs-pr-audit"');
    }

    if (!/^[A-Za-z0-9._-]+$/.test(name!)) {
        fail('NAME must use only A-Z a-z 0-9 . _ -');
    }

    const { ts, iso } = generateTimestamp();
    const runId = `${ts}-${name}`;
    const runDir = path.join(process.cwd(), "runs", runId);

    if (fs.existsSync(runDir)) {
        fail(`Run already exists: ${runDir}`);
    }
...
`;
    ...
    const requestContent = `# Task request
...
`;
    ...
    console.log(`Created run: ${runDir}/`);
}
