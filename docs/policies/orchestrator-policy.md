...
The orchestrator wrappers still expect the legacy planner interface that accepted `--goal/--context`. The current ADX CLI reads from `runs/<RUN>/inputs/` via `pnpm exec agentic plan --run "<RUN>"`. Until the orchestrator is updated, prefer running the planner manually with run inputs (see README and Golden Path docs) and treat the orchestrator targets as legacy.
