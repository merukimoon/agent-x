...
- Stable: canonical entry commands (`make verify`, `make verify-plan-e2e`, `make verify-flow`, `make orchestrator-validate`, `npm run verify-run -- --run <RUN>` / `make validate-run RUN=<RUN>`), and status semantics in `docs/status.md`.
...
- Run validation: `make validate-run RUN=<RUN_ID>` (alias: `make verify-run RUN=<RUN_ID>`, `npm run verify-run -- --run <RUN_ID>`)
