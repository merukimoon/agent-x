...
Additionally, documentation layout created a discoverability gap: agent specifications lived in `agents/`, but the canonical "how-to-run" guides (Golden Paths) lived in `examples/` or `docs/`, with no direct linkage between specification and implementation.
...
4. **Linkage Mandate**: Agent specifications (e.g., `agents/planner/README.md`) must explicitly link to their canonical usage guides (Golden Paths) and policy documentation.
