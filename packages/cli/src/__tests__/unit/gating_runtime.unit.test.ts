...
import type { GateOutcome, Strictness } from "../../core/src/policy/gating";
...
const makeFakeDeps = () => {
  ...
  const fsOps = {
    existsSync: (filePath: string) => normalizePath(filePath) in files,
    statSync: (filePath: string) => ({ isFile: () => normalizePath(filePath) in files }),
...
describe("gating runtime helpers", () => {
  ...
  it("applies overrides only when valid", () => {
    ...
    const invalidBase = { ...baseDecision, decision: { action: "continue", reason: "ok" } };
