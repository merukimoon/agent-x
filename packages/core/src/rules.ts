 ...
 * @returns {RulePack[]}
 */
export function loadRulePacks(): RulePack[] {
 ...
 * @param {string} source
 */
export function validateRulePack(pack: unknown, source: string): asserts pack is RulePack {
    if (
        !pack ||
        typeof pack !== "object" ||
        typeof (pack as RulePack).flow_type !== "string" ||
        !Array.isArray((pack as RulePack).keywords) ||
        !Array.isArray((pack as RulePack).steps)
    ) {
        fail(`Rule pack invalid at ${source}`);
    }
    const asPack = pack as RulePack;
 ...
 */
export function classifyFlow(requestText: string, contextText: string): { pack: RulePack; signals: string[]; confidence: ConfidenceLevel } {
    ...
    let best: { pack: RulePack; matches: string[] } | null = null;
    let bestCount = 0;
    ...
    const chosen = best as { pack: RulePack; matches: string[] };
